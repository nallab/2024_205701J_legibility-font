...
class ImageAttr(data.Dataset):
    ...
    def __init__(self, image_dir, attr_path, transform, mode,
                 binary=False, n_style=4, style_char_path = '',
                 char_num=52, unsuper_num=968, train_num=120, val_num=28, fix_index=0):
        ...
        self.style_char_path = style_char_path
        self.style_char_dict = None
        ...
        if mode == 'train':
            ...
            self.num_images = len(self.unsuper_train_dataset)
        elif mode == 'test':
            self.num_images = len(self.super_test_dataset)

    def preprocess(self):
        self.create_style_char_dict()
        ...
        if (self.mode == 'fix'):
            filename_A, _, _, _ = self.unsuper_train_dataset[self.fix_index]
            print('fix font:', filename_A)
        print('Finished preprocessing the Image Attribute (Explo) dataset...')

    def create_style_char_dict(self):
        if (not os.path.exists(self.style_char_path)):
            return

        num2char = list(string.digits+string.ascii_lowercase+string.ascii_uppercase)
        char2num = {num2char[i]: i for i in range(0, len(num2char))}
        num2upper_num = lambda num: num+len(string.ascii_lowercase)

        with open(self.style_char_path) as f:
            reader = csv.reader(f)
            l = [row for row in reader]
        head = l.pop(0)

        self.style_char_dict = defaultdict(dict)
        for row in l:
            font_name = row[0]
            actual_char = row[1]
            sim_chars = list(map(lambda c: char2num[c], row[2:].copy()))
            # sim_chars = [char2num[row[2]], char2num[row[3]], num2upper_num(char2num[row[2]]), num2upper_num(char2num[row[3]])]

            self.style_char_dict[font_name][char2num[actual_char]] = sim_chars.copy()
            self.style_char_dict[font_name][num2upper_num(char2num[actual_char])] = sim_chars.copy()
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            index = index%52 + self.fix_index
            filename_A, charclass_A, fontclass_A, attr_A = self.unsuper_train_dataset[index]
            label_A = 0.0
            font_embed_A = int(index/self.char_num)  # convert to [0, 967]

            filename_B, charclass_B, fontclass_B, attr_B = self.unsuper_train_dataset[index]
            label_B = 1.0
            font_embed_B = self.unsupervised_font_num  # dummy id 968
        ...
        if (not self.style_char_dict == None):
            font_name: str = filename_A[:-7]
            char_num = int(filename_A[-6:-4])
            style_chars = self.style_char_dict[font_name][char_num]
        styles_A = []
        if self.n_style == 1:
        ...
        style_chars = self.chars[:self.n_style]
        if (not self.style_char_dict == None):
            font_name: str = filename_B[:-7]
            char_num = int(filename_B[-6:-4])
            style_chars = self.style_char_dict[font_name][char_num]
        styles_B = []
        if self.n_style == 1:
        ...
        return {"img_A": self.transform(image_A), "charclass_A": torch.LongTensor([charclass_A]),
                ...
                "styles_A": style_imgs_A, "filename_A": filename_A,
                ...
                "styles_B": style_imgs_B, "filename_B": filename_B,
...
def get_loader(image_dir, attr_path, image_size=256,
    ...
    if dataset_name == 'explor_all':
        dataset = ImageAttr(image_dir, attr_path, transform,
                            mode, binary, n_style, style_char_path,
                            char_num=52, unsuper_num=968,
                            train_num=120, val_num=28, fix_index=fix_index)

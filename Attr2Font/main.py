...
def fix_key(state_dict):
    ...
    # before =  ["style_enc.res_layer.0.conv_block.2.weight", "style_enc.res_layer.1.conv_block.2.weight", "style_enc.res_layer.2.conv_block.2.weight", "style_enc.res_layer.3.conv_block.2.weight", "style_enc.res_layer.4.conv_block.2.weight", "style_enc.res_layer.5.conv_block.2.weight", "style_enc.res_layer.6.conv_block.2.weight", "style_enc.res_layer.7.conv_block.2.weight", "style_enc.res_layer.8.conv_block.2.weight", "style_enc.res_layer.9.conv_block.2.weight", "style_enc.res_layer.10.conv_block.2.weight", "style_enc.res_layer.11.conv_block.2.weight", "style_enc.res_layer.12.conv_block.2.weight", "style_enc.res_layer.13.conv_block.2.weight", "style_enc.res_layer.14.conv_block.2.weight", "style_enc.res_layer.15.conv_block.2.weight"]
    # after = ["style_enc.res_layer.0.conv_block.3.weight", "style_enc.res_layer.1.conv_block.3.weight", "style_enc.res_layer.2.conv_block.3.weight", "style_enc.res_layer.3.conv_block.3.weight", "style_enc.res_layer.4.conv_block.3.weight", "style_enc.res_layer.5.conv_block.3.weight", "style_enc.res_layer.6.conv_block.3.weight", "style_enc.res_layer.7.conv_block.3.weight", "style_enc.res_layer.8.conv_block.3.weight", "style_enc.res_layer.9.conv_block.3.weight", "style_enc.res_layer.10.conv_block.3.weight", "style_enc.res_layer.11.conv_block.3.weight", "style_enc.res_layer.12.conv_block.3.weight", "style_enc.res_layer.13.conv_block.3.weight", "style_enc.res_layer.14.conv_block.3.weight", "style_enc.res_layer.15.conv_block.3.weight"]
    # for b,a in zip(before, after):
    #     if (b in new_state_dict):
    #         new_state_dict[a] = new_state_dict.pop(b)
...
def train(opts):
    ...
    attribute_path = os.path.join(opts.data_root, opts.dataset_name, opts.attributes_file)
    style_char_path = os.path.join(opts.data_root, opts.dataset_name, opts.style_char_file)
    ...
    train_dataloader = get_loader(image_dir, attribute_path,
                                  ...
                                  style_char_path=style_char_path,
    ...
    # FineTune
    if (opts.fine_tune_dir != 'None'):
        gen_file = os.path.join(opts.fine_tune_dir, f"G_{opts.fine_tune_init_epoch}.pth")
        print('load:',gen_file)
        attr_unsuper_file = os.path.join(opts.fine_tune_dir, f"attr_unsuper_embed_{opts.fine_tune_init_epoch}.pth")
        print('load:',attr_unsuper_file)
        attribute_embed_file = os.path.join(opts.fine_tune_dir, f"attribute_embed_{opts.fine_tune_init_epoch}.pth")
        print('load:',attribute_embed_file)
        dis_file = os.path.join(opts.fine_tune_dir, f"D_{opts.fine_tune_init_epoch}.pth")
        print('load:',dis_file)

        generator.load_state_dict(fix_key(torch.load(gen_file)))
        attr_unsuper_tolearn.load_state_dict(fix_key(torch.load(attr_unsuper_file)))
        attribute_embed.load_state_dict(fix_key(torch.load(attribute_embed_file)))
        discriminator.load_state_dict(fix_key(torch.load(dis_file)))
...
def genImg(opts):
    ...
    log_dir = os.path.join("experiments", opts.experiment_name)
    checkpoint_dir = os.path.join(log_dir, "checkpoint")
    results_dir = os.path.join(log_dir, "outputs")

    attrB_file = opts.attrB
    ...
    dataloader = get_loader(image_dir, attribute_path,
                                 dataset_name=opts.dataset_name,
                                 image_size=opts.img_size,
                                 n_style=opts.n_style, batch_size=52,
                                 style_char_path=style_char_path,
    ...
    gen_file = os.path.join(checkpoint_dir, f"G_{opts.genImg_epoch}.pth")
    attribute_embed_file = os.path.join(checkpoint_dir, f"attribute_embed_{opts.genImg_epoch}.pth")
    attr_unsuper_file = os.path.join(checkpoint_dir, f"attr_unsuper_embed_{opts.genImg_epoch}.pth")
    ...
    if torch.cuda.is_available():
        ...
        generator.load_state_dict(torch.load(gen_file, torch.device('cpu')))
        attribute_embed.load_state_dict(torch.load(attribute_embed_file, torch.device('cpu')))
        attr_unsuper_tolearn.load_state_dict(torch.load(attr_unsuper_file, torch.device('cpu')))
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            img_sample = torch.cat((img_A.data, fake_B.data), -2)
            save_file = os.path.join(results_dir, f"output_{batch_idx}.png")
            print(f"save image at {results_dir}")
            save_image(img_sample, save_file, nrow=52, normalize=True)
            break

def getAttrB(attrB_file_path):
    lines = [line.rstrip() for line in open(attrB_file_path, 'r')]
    line = lines[0]
...
def main():
    ...
    if opts.phase == 'train':
        # Create directories
        log_dir = os.path.join("experiments", opts.experiment_name)
        is_train_resume = opts.init_epoch > 1
        os.makedirs(log_dir, exist_ok=is_train_resume)  # False to prevent multiple train run by mistake
        ...
        interp(opts)
    elif opts.phase == 'gen_img':
        print("Generating font image...")
        genImg(opts)
    else:
